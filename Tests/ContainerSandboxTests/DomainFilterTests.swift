...
struct DomainFilterTests {
    ...
    @Test func denyModeBlocksUnlisted() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: ["api.anthropic.com"]))
    ...
    @Test func wildcardMatchesSubdomain() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: ["*.anthropic.com"]))
        #expect(filter.evaluate(host: "api.anthropic.com", port: 443) == .allow)
    ...
    @Test func portSpecificMatch() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: ["api.anthropic.com:443"]))
        #expect(filter.evaluate(host: "api.anthropic.com", port: 443) == .allow)
        #expect(filter.evaluate(host: "api.anthropic.com", port: 80) != .allow)
    }

    @Test func wildcardWithPort() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: ["*.anthropic.com:443"]))
        #expect(filter.evaluate(host: "api.anthropic.com", port: 443) == .allow)
        #expect(filter.evaluate(host: "api.anthropic.com", port: 80) != .allow)
    }
    ...
    @Test func allowModeAllowsUnlisted() {
        let policy = NetworkPolicy.filtered(
            allowedHosts: [],
            direction: .allow,
    ...
    @Test func ipv4CIDRBlocking() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: []))
    ...
    @Test func multipleAllowedHosts() {
        let filter = DomainFilter(policy: .filtered(allowedHosts: [
            "*.anthropic.com", "*.github.com", "registry.npmjs.org",
        ]))
        #expect(filter.evaluate(host: "api.anthropic.com", port: 443) == .allow)
